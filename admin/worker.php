...
$stmt = $pdo->prepare("select * from workers order by id desc");
$stmt->execute();
$result1 = $stmt->fetchAll();

$total_page = ceil(count($result1) / $numOfrecs);
$stmt1 = $pdo->prepare("select * from workers order by id desc limit $offset,$numOfrecs");
$stmt1->execute();
                ...
                <div class="col-12 col-md-10 col-lg-10">
                    <div class="card">
                        <div class="card-header">
                            <a href="createworker.php" class="btn btn-success">Create</a>
                        </div>
                                ...
                                <table class="table table-striped table-md">
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <?php
                                            $id = $value['service_id'];
                                            $stmtservice = $pdo->prepare('select title from services where id=' . $id);
                                            $stmtservice->execute();
                                            ...
                                            <td><a href="edit-worker.php?id=<?php echo $value['id'] ?>" class="btn btn-primary">Edit</a>
                                                <a href="delete-worker.php?id=<?php echo $value['id'] ?>" class="btn btn-danger" onclick="return confirm('Are you sure to delete')">Delete</a>
                                            </td>
                                        </tr>
                                    <?php
                                        $i++;
