...
if ($_POST) {
    ...
    if (empty($username) || empty($email) || empty($subject) || empty($message)) {
        ...
        $stmt = $pdo->prepare("insert into contacts (username,email,subject,message) values(:username,:email,:subject,:message)");
        $result = $stmt->execute(
            array(
                ':username' => $username,
                ':email' => $email,
        ...
        if ($result) {
            echo "<script>alert('Successfully');window.location.href='contact.php'
            </script>";
        }
    }
}
